...
from ddpm.cnn_featurizer import Inception3DEncoder
...
class SinusoidalCondEncoder(nn.Module):
    def __init__(self, num_features, embedding_dim):
        """
        Args:
        - num_features (int): Number of real-valued features to encode.
        - embedding_dim (int): Dimensionality of the sinusoidal encoding for each feature.
        """
        super().__init__()
        self.num_features = num_features
        self.embedding_dim = embedding_dim
        self.scale_factors = (10000 ** (torch.arange(0, embedding_dim, 2).float() / embedding_dim)).cuda()

    def forward(self, features):
        """
        Args:
        - features (torch.Tensor): Tensor of shape (batch_size, num_features) containing real values.
        
        Returns:
        - encoded_features (torch.Tensor): Tensor of shape (batch_size, num_features * embedding_dim).
        """
        batch_size, num_features = features.shape
        assert num_features == self.num_features, f"Mismatch in number of features, {features.shape}"
        # print(f"features device: {features.device}, scale_factors device: {self.scale_factors.device}")
        encoded_features = []
        for i in range(num_features):
            feature = features[:, i].unsqueeze(1)  # Shape: (batch_size, 1)
            # print(f"feature device: {feature.device}, scale_factors device: {self.scale_factors.device}")
            scaled_feature = feature / self.scale_factors  # Broadcasting to (batch_size, embedding_dim // 2)
            sin_feature = torch.sin(scaled_feature)
            cos_feature = torch.cos(scaled_feature)
            encoded_feature = torch.cat([sin_feature, cos_feature], dim=-1)  # Shape: (batch_size, embedding_dim)
            encoded_features.append(encoded_feature)
        
        # Concatenate encodings for all features along the last dimension
...
class Unet3D(nn.Module):
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        print("In Forward With Cond Scale, Using Cond Scale", cond_scale)
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sin_cond_encoder = SinusoidalCondEncoder(num_features=num_features, embedding_dim=cond_dim).cuda()
        self.cnn_encoder = Inception3DEncoder(use_pretrained=True)
    ...
    @torch.inference_mode()
    def p_sample_loop(self, shape, cond=None, cond_scale=1., timesteps=None):
    ...
    @torch.inference_mode()
    def sample(self, cond_feat=None, cond_scale=2., batch_size=16, timesteps=None, denorm=False):
        device = next(self.denoise_fn.parameters()).device
        cond = self.sin_cond_encoder(cond_feat.to(device))
        # cond = None
        ...
        print("In sample shapes", (batch_size, channels, num_frames, image_size, image_size))
        _sample = self.p_sample_loop(
            (batch_size, channels, num_frames, image_size, image_size), cond=cond, cond_scale=cond_scale, timesteps=timesteps)

        if isinstance(self.vqgan, VQGAN):
            print("in sample", device, _sample.device, self.vqgan.codebook.embeddings.max().device)
            ...
            print("Sample dim before decode: ", _sample.shape)
            _sample = self.vqgan.decode(_sample, quantize=True)
            if denorm:
                print("Denormalizing Image...")
                _sample = (_sample + 1.0) * 127.5
            print("Sample dim after decode: ", _sample.shape)
        else:
    ...
    def p_losses(self, x_start, t, cond=None, noise=None, **kwargs):
        ...
        x_recon = self.denoise_fn(x_noisy, t, cond=cond, null_cond_prob=self.null_cond_prob, **kwargs)

        # print("In p_losses: x_start, x_noisy, x_recon", x_start.shape, x_noisy.shape, x_recon.shape)
    ...
    def forward(self, x, y, *args, **kwargs):
        ...
        y = y * INPUT_SCALE_FACTOR
        
        if self.counter % 100 == 0:
            print(f"\n=========Sanity Checking for Cond, In forward, y={y}=======\n")
        cond = self.sin_cond_encoder(y)
        
        # print("In forward: x, cond", x.shape, cond.shape)
        
        with torch.no_grad():
            x = self.vqgan.encode(
                x, quantize=False, include_embeddings=True)
            # print("In forward: before normalization", x.shape)
...
def video_tensor_to_grid(tensor, path):
    print("video_tensor_to_grid", tensor.shape)
    t2_images = tensor[0, 0::3, :, :]
    adc_images = tensor[0, 1::3, :, :]
    dwi_images = tensor[0, 2::3, :, :]
    
     # Create a grid for each modality
    t2_grid = torchvision.utils.make_grid(t2_images.view(-1, 1, 132, 132), nrow=8)
    adc_grid = torchvision.utils.make_grid(adc_images.view(-1, 1, 132, 132), nrow=8)
    dwi_grid = torchvision.utils.make_grid(dwi_images.view(-1, 1, 132, 132), nrow=8)
    
    combined_grid = torch.cat((t2_grid, adc_grid, dwi_grid), dim=1)
    combined_grid = combined_grid.transpose(0, 1).transpose(1, 2).squeeze(-1)
    combined_grid = combined_grid.cpu().numpy().astype(np.uint8)
...
class Trainer(object):
    def __init__(
        ...
        print(f"Init Trainer: rank {rank}")
        self.rank = rank
        self.device = torch.device(f'cuda')
        
        self.denorm = denorm
        # self.model = diffusion_model.to(self.device)
        self.model = DDP(diffusion_model, device_ids=[self.rank], output_device=self.rank, find_unused_parameters=True)
        self.ema = EMA(ema_decay)
        self.ema_model = copy.deepcopy(diffusion_model).to(self.device)
        self.step = 0
        self.scaler = GradScaler(enabled=amp)
    
        if load_path:
            print(f"init: Loading from milestone: {load_path}")
            self.load(load_path)
        ...
        self.sampling_csv = sampling_csv
        
        if self.sampling_csv:
            self.sampling_df = pd.read_csv(self.sampling_csv)
        ...
        self.sampler = DistributedSampler(self.ds, num_replicas=world_size, rank=rank)
        # self.dl = DataLoader(self.ds, batch_size=train_batch_size, shuffle=True, pin_memory=True, num_workers=num_workers)
        self.dl = DataLoader(self.ds, batch_size=train_batch_size, sampler=self.sampler, pin_memory=True, num_workers=num_workers)

        self.len_dataloader = len(self.dl)
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            for original, y in self.dl:
                # original, y = next(self.dl)
                ...
                if self.step % self.update_ema_every == 0 and self.rank == 0:
                    self.step_ema()
                ...
                if self.rank == 0 and self.step != 0 and self.step % self.save_and_sample_every == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if self.sampling_csv:
                            n=1
                            sampled_rows = self.sampling_df[['cancer', 'psa', 'dataset_gland_vol', 'age']].sample(n=n)
                            # sampled_rows = self.sampling_df[['cancer']].sample(n=n)
                            milestone_text = f"sample-{milestone}-cancer-{sampled_rows['cancer'].mean()}"
                            # sampled_rows['cancer'] = sampled_rows['cancer'].astype(int)
                            cond_feat = torch.tensor(sampled_rows.values, dtype=torch.float32).cuda()
                            cond_feat[:, 0] *= INPUT_SCALE_FACTOR  # scale only the first column
                            print("Cond Feat", cond_feat)
                            video = self.ema_model.sample(cond_feat=cond_feat, batch_size=n, denorm=self.denorm)
                        else:
                            video = self.ema_model.sample(batch_size=n, denorm=self.denorm)
                        all_videos_list.append(video)
                            
                        # all_videos_list = list(
                        #     map(lambda n: self.ema_model.sample(batch_size=n), batches))
                        # save npy of all videos list
                        all_videos_list = torch.cat(all_videos_list, dim=0)
                        # npy_path = str(self.results_folder / f'sample-{milestone}.npy')
                        # np.save(npy_path, all_videos_list.cpu().numpy())
                        print("All videos list: ", all_videos_list.shape)

                    all_videos_list = F.pad(all_videos_list, (2, 2, 2, 2))

                    one_gif = rearrange(
                        all_videos_list, '(i j) c f h w -> c f (i h) (j w)', i=self.num_sample_rows)
                    video_path = str(self.results_folder / str(f'{milestone}-{milestone_text}.gif'))
                    video_grid_path = str(self.results_folder / str(f'{milestone}-{milestone_text}_grid.png'))
                    video_tensor_to_gif(one_gif, video_path)
                    video_tensor_to_grid(one_gif, video_grid_path)
