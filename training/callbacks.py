...
from vq_gan.utils import save_video_grid

class ImageLoggerMatrix(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        print(f"Log: logging images from batch {batch_idx}, {save_dir}")
        for k in images:
            print("Images Log Shape", images[k].shape)
            images[k] = (images[k] + 1.0) * 127.5  # std + mean
            image_array = images[k].cpu().numpy()  # Convert tensor to numpy array
            torch.clamp(images[k], 0, 255)

            # Extract modalities
            t2_images = images[k][:, :, 0::3, :, :]
            adc_images = images[k][:, :, 1::3, :, :]
            dwi_images = images[k][:, :, 2::3, :, :]

            # Create a grid for each modality
            t2_grid = torchvision.utils.make_grid(t2_images.view(-1, 1, 128, 128), nrow=3)
            adc_grid = torchvision.utils.make_grid(adc_images.view(-1, 1, 128, 128), nrow=3)
            dwi_grid = torchvision.utils.make_grid(dwi_images.view(-1, 1, 128, 128), nrow=3)

            # Combine grids into a single image
            combined_grid = torch.cat((t2_grid, adc_grid, dwi_grid), dim=1)
            combined_grid = combined_grid.transpose(0, 1).transpose(1, 2).squeeze(-1)
            combined_grid = combined_grid.numpy().astype(np.uint8)
...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        print(f"Log: logging images from batch {batch_idx}, {save_dir}")
        for k in images:
            # images[k] = images[k] * 255.0
            images[k] = (images[k] + 1.0) * 127.5  # std + mean
            image_array = images[k].cpu().numpy()  # Convert tensor to numpy array
            # numpy_array_path = os.path.join("debug/images", f"{k}_clamped_gs-{global_step:06}_e-{current_epoch:06}_b-{batch_idx:06}.npy")
            # np.save(numpy_array_path, image_array)  # Save as .npy file
            torch.clamp(images[k], 0, 255)
...
class VideoLogger(Callback):
    ...
    def log_local(self, save_dir, split, videos,
        ...
        for k in videos:
            videos[k] = (videos[k] + 1.0) * 127.5  # std + mean
            image_array = videos[k].cpu().numpy()  # Convert tensor to numpy array
            torch.clamp(videos[k], 0, 255)

            # Extract modalities
            t2_images = videos[k][0, :, 0::3, :, :]
            adc_images = videos[k][0, :, 1::3, :, :]
            dwi_images = videos[k][0, :, 2::3, :, :]
