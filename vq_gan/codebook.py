...
from vq_gan.utils import shift_dim
...
class Codebook(nn.Module):
    ...
    def _tile(self, x):
        """
        Tiles the input tensor x to match the number of codes in the codebook.
        If the number of rows in x is less than the number of codes, it repeats
        the rows to match the number of codes. Additionally, it adds a small
        amount of noise to the repeated rows to introduce randomness.

        Args:
            x (torch.Tensor): The input tensor to be tiled.

        Returns:
            torch.Tensor: The tiled tensor with added noise.
        """
        d, ew = x.shape
    ...
    def _init_embeddings(self, z):
        ...
        # output - atleast [n_codes, c]
        y = self._tile(flat_inputs)

        d = y.shape[0]
        # move things around and take the first n_codes
        # _k_rand = [n_codes, c]
        _k_rand = y[torch.randperm(y.shape[0])][:self.n_codes]
        if dist.is_initialized():
            dist.broadcast(_k_rand, 0)
        self.embeddings.data.copy_(_k_rand)
        print(f"\n\n init embeddings: {self.embeddings.shape}, {_k_rand.shape} \n\n")
