...
def shift_dim(x, src_dim=-1, dest_dim=-1, make_contiguous=True):
    """
    Shifts the dimension at src_dim to the position at dest_dim in the tensor x.

    If src_dim or dest_dim is negative, it counts from the end of the dimensions.
    For example, shift_dim(x, 1, -1) would transform a tensor from shape (b, c, t, h, w) to (b, t, h, w, c).

    Args:
        x (torch.Tensor): The input tensor.
        src_dim (int, optional): The source dimension to shift. Defaults to -1.
        dest_dim (int, optional): The destination dimension. Defaults to -1.
        make_contiguous (bool, optional): If True, the output tensor is made contiguous in memory. Defaults to True.

    Returns:
        torch.Tensor: The tensor with the dimension shifted.
    """
    n_dims = len(x.shape)
    if src_dim < 0:
